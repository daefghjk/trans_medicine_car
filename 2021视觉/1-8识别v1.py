            ...
            if rgb888p_img.format() == image.RGBP888:
                ...
                if det_boxes:
                    for det_boxe in det_boxes:
                        ...
                        # 新增：置信度大于0.7时，串口发送、LCD左上角显示、终端打印
                        if det_boxe[1] > 0.7:
                            number = labels[det_boxe[0]]
                            confidence = det_boxe[1]
                            # 调用独立的串口发送函数
                            send_uart_number(uart, number, confidence)
                            # 收集显示文本
                            display_texts.append(f"{number} {confidence:.2f}")
                            # 终端打印
                            print(f"识别数字: {number}, 置信度: {confidence:.2f}")
                # 左上角显示所有大于0.7的识别结果
                for idx, txt in enumerate(display_texts):
                    osd_img.draw_string_advanced(5, 5 + idx * 36, 32, txt, color=(255,0,0))
